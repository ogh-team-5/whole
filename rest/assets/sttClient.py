...
class WSInterfaceProtocol(WebSocketClientProtocol):
   ...
   def setUtterance(self, utt):
      ...
      self.listJsonPkl = self.dirOutput + "/" + str(self.uttNumber) + ".json.pkl"
      self.listJson = []
      try:
      ...
      if isBinary:
         ...
         # if in streaming 
            ...
            # regular hypothesis
               ...
               f.close()
               self.listJson.append(jsonObject)
               import pickle
               pickle.dump(self.listJson, open(self.listJsonPkl, "wb"))
